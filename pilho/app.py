...
def load_and_prepare_data():
    ...
    features = [c for c in df.select_dtypes(include=[np.number]).columns if c not in ['kospi_close', 'log_return', 'log_return_t+1', 'target', 'KFGI']]
    X = df[features]
    y = df['target']
    
    model_ridge = Ridge(alpha=10.0).fit(X, y)
    model_lgb = lgb.LGBMRegressor(n_estimators=100, learning_rate=0.05, random_state=42, verbose=-1).fit(X, y)
    df['pred'] = (model_ridge.predict(X) * 0.2) + (model_lgb.predict(X) * 0.8)
    
    threshold = df['pred'].quantile(0.05)
    df['bull_signal'] = np.where(df['pred'] > threshold, 1, 0)
    df['bull_weight'] = df['KFGI'].apply(lambda x: 2.5 if x < 25 else (1.0 if x > 75 else 1.2))
    df['bull_ret'] = df['bull_signal'] * df['bull_weight'] * df['target']
    
    FEES = 0.00015
    def get_hybrid_weight(row):
        if row['market_trend'] == 1: return 1.8 if row['pred'] > 0 else 1.0
        else: return 0.3 if row['pred'] > 0.002 else 0.0
    df['hyb_weight'] = df.apply(get_hybrid_weight, axis=1)
    df['hyb_turnover'] = df['hyb_weight'].diff().abs().fillna(1.8)
    df['hyb_ret'] = (df['hyb_weight'] * df['target']) - (df['hyb_turnover'] * FEES)
...
with st.sidebar.form(key='setup_form'):
    strategy_choice = st.radio("1️⃣ 투자 전략 선택", ('하이브리드 (안정/방어형)', '불도저 (공격/위험감수형)'))
...
if not st.session_state.analysis_started:
    ...
    strat_col = 'hyb_ret' if '하이브리드' in strategy_choice else 'bull_ret'
    ...
    next_weight = latest_data['hyb_weight'] if '하이브리드' in strategy_choice else latest_data['bull_weight']
