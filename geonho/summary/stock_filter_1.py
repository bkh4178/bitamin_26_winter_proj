...
def run_stock_filter(year: int):
    ...
    # 1. 데이터 로딩
    # --------------------------
    input_file = Path(f"data/NAVER/toxicity/comments_toxicity_kept_{year}_1.csv")
    ...
    with open(output_dir / f"keyword_analysis_result_{year}_1.json", "w", encoding="utf-8") as f:
        json.dump(keyword_result, f, ensure_ascii=False, indent=2)

    df.to_csv(output_dir / f"classified_stock_comments_{year}_1.csv", index=False, encoding="utf-8-sig")
    stock_df.to_csv(output_dir / f"comments_final_stock_only_{year}_1.csv", index=False, encoding="utf-8-sig")

    stock_df.drop(columns=['stock_score', 'is_stock'], errors='ignore').to_csv(
        output_dir / f"comments_stock_clean_{year}_1.csv",
        index=False,
        encoding="utf-8-sig"
    )

    other_df.to_csv(output_dir / f"comments_other_{year}_1.csv", index=False, encoding="utf-8-sig")
    ...
    with open(output_dir / f"stock_classification_summary_{year}_1.json", "w", encoding="utf-8") as f:
