...
def run_politic_filter(year: int, start_date: str):

    print("=" * 80)
    print(f"{year} 정치 키워드 기반 댓글 필터 시작")
    print("=" * 80)

    input_file = f"data/NAVER/comments/comments_{year}_4_{start_date}_top5.csv"
    ...
    # 전체 분류 저장
    df.to_csv(
        f"{output_dir}/comments_{year}_4_{start_date}_classified.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    political_comments.to_csv(
        f"{output_dir}/comments_{year}_4_{start_date}_political_only.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    non_political_comments.to_csv(
        f"{output_dir}/comments_{year}_4_{start_date}_non_political_only.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    cleaned.to_csv(
        f"{output_dir}/comments_political_removed_{year}_4_{start_date}.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    with open(f"{output_dir}/classification_summary_{year}_4_{start_date}.json", "w", encoding="utf-8") as f:
