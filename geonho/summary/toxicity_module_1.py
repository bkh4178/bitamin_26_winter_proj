...
def run_toxicity(
    ...
    # 입력: 정치 필터 제거된 파일
    inp = Path(f"data/NAVER/political_filter/comments_political_removed_{year}_1.csv")

    # 출력: toxicity 결과
    out_dir = Path("data/NAVER/toxicity")
    out_dir.mkdir(parents=True, exist_ok=True)

    out_all = out_dir / f"comments_toxicity_{year}_1.csv"
    out_kept = out_dir / f"comments_toxicity_kept_{year}_1.csv"
    out_dropped = out_dir / f"comments_toxicity_dropped_{year}_1.csv"
